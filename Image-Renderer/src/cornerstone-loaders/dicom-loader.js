...
(function (cs) {
    ...
    function getDICOMImage(imageId) {
       
        const id = imageId.split('dicomLoader://')[1]

        return new Promise((resolve, reject) => {
            axios({
                method: 'GET',
                responseType: 'json',
                url: `/api/dicom-bytes/${id}`,
            }).then(response => {
    ...
    cs.registerImageLoader('dicomLoader', getDICOMImage);
